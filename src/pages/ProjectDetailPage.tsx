...
const EDITORIAL_PLACEHOLDERS = {
  ...
  problem:
    "Problem placeholder: describe the core communication challenge, risk, or friction the work needed to solve.",
  strategy:
    "Strategy placeholder: describe the narrative, design system, and production choices that guided the solution.",
  tradeoffs:
    "Trade-offs placeholder: describe the constraints, compromises, and decisions that shaped the final direction.",
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between gap-4 px-6 md:px-10 pt-6 pb-4 pointer-events-none">
        ...
        >
          {title && (
            <span className="font-display text-sm md:text-base text-white/90 truncate max-w-[60vw]">{title}</span>
          )}
...
const BENTO_LAYOUTS: Record<number, BentoLayout> = {
  ...
  3: {
    ...
    cols: "md:grid-cols-3",
    tiles: ["md:col-span-2 md:row-span-2", "md:col-span-1 md:row-span-1", "md:col-span-1 md:row-span-1"],
  },
  4: {
    // 2 × 2
    cols: "md:grid-cols-2",
          ...
          {img.caption && (
            <>
              ...
              <div className="pointer-events-none absolute inset-x-0 bottom-0 p-4 md:p-5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <p className="text-white text-sm md:text-[15px] leading-snug text-left">{img.caption}</p>
              </div>
            </>
          )}
        </button>
...
const BeforeAfterSlider = ({ before, after }: { before: string; after: string }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 bottom-0 z-20" style={{ left: `${pos}%`, transition: sharedTransition }}>
        ...
        <AnimatePresence>
          {showTip && (
            <motion.div
              ...
              className="absolute top-1/2 left-1/2 -translate-x-1/2 translate-y-4 z-30 pointer-events-none whitespace-nowrap rounded-full bg-foreground text-background text-[11px] tracking-[0.18em] uppercase px-3 py-1.5 shadow-lg"
              role="status"
            >
              Drag to compare
            </motion.div>
...
/* ------------------------------------------------------------------ */
const LiquidGlassDots = ({ total, idx, onSelect }: { total: number; idx: number; onSelect: (i: number) => void }) => {
            ...
            >
              <span
                className={`block rounded-full transition-all duration-300 ${
                  active ? "w-2 h-2 bg-white" : "w-1.5 h-1.5 bg-white/40 group-hover:bg-white/70"
                }`}
              />
            </button>
          );
...
const ProjectDetailPage = () => {
  ...
  // Derived/fallback content - PLUGADO DIRETO NO MOTOR (images.ts)
  const derived = useMemo(() => {
    if (!project || !projectId) return null;

    // 🔥 A FONTE DA VERDADE: Puxando as fotos direto do nosso motor perfeito
    ...
    if (!bentoGalleries || bentoGalleries.length === 0) {
      bentoGalleries =
        project.bentoGalleries && project.bentoGalleries.length > 0
          ? project.bentoGalleries
          : [
              {
                id: `${project.id}-gallery`,
                label: project.galleryLabel ?? project.title,
                images: project.processImages?.length ? project.processImages : project.images.map((src) => ({ src })),
              },
            ];
    }
    ...
    const skills = project.skills && project.skills.length > 0 ? project.skills : [];
      ...
      <PageLayout>
        <main className="pt-28 pb-16">
          <div className="container mx-auto px-6 text-center">
            ...
            <Link to="/work" className="inline-flex items-center gap-2 text-primary hover:gap-3 transition-all">
    ...
    <PageLayout>
      ...
      <main className="pb-24">
        ...
        <section className="content-shell pt-8 md:pt-10">
          ...
          <p className="mt-8 font-sans text-base md:text-lg leading-relaxed text-foreground">{derived.tldr}</p>
        ...
        {derived.beforeAfter && (
          <section className="content-shell pt-12 md:pt-14">
            <BeforeAfterSlider before={derived.beforeAfter.before} after={derived.beforeAfter.after} />
          </section>
        )}
          ...
          <div className="relative w-full rounded-md overflow-hidden isolate" style={{ background: "#f5f5f5" }}>
            <div className="relative z-10 flex flex-col items-center justify-center text-center px-6 md:px-12 py-7 md:py-9">
              ...
              <p className="text-muted-foreground text-sm md:text-base max-w-xl mb-3 md:mb-4 leading-relaxed">
                Thank you for showing interest in my work. If you'd like to learn more about this project, please reach
                out!
              </p>
              <button
                type="button"
      ...
      {lightbox && (
        <Lightbox
          ...
          onNext={() => setLightbox((l) => (l ? { ...l, index: (l.index + 1) % l.images.length } : l))}
        />
      )}
    </PageLayout>
  );
